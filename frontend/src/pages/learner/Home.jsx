  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {primaryPath && (
        <section className="rounded-2xl border border-black/5 bg-white p-6">
          ...
          <p className="mt-2 text-xs text-neutral-400">
            {primaryPath.progress.modulesCompleted} / {primaryPath.progress.totalModules} modules
          </p>
        </section>
      )}
      ...
      <section className="rounded-2xl border border-black/5 bg-white p-6">
        ...
        {badges.length === 0 ? (
          <p className="mt-3 text-sm text-neutral-400">No badges earned yet — complete a module to get started.</p>
        ) : (
      ...
      <section className="rounded-2xl border border-black/5 bg-white p-6">
        ...
        {topLearners.length === 0 ? (
          <p className="mt-3 text-sm text-neutral-400">Leaderboard is empty for now.</p>
        ) : (
          <div className="mt-3 flex flex-col">
            {topLearners.map((learner) => (
              <div key={learner.id} className="flex items-center gap-4 border-b border-neutral-100 py-3 last:border-0">
                <span className="w-5 text-sm text-neutral-400">{learner.rank}</span>
                ...
                <span className={`flex-1 text-sm font-medium ${learner.id === user?.id ? "text-purple-600" : "text-neutral-900"}`}>
