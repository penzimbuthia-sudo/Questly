...
import { Search, Plus, Edit, Trash2, Users, Target, Flame, Crown, CheckCircle, FileText, MapPin, ThumbsUp, Heart, X } from 'lucide-react';
...
const initialBadges = [
  ...
  { id: 3, title: 'Quiz champion', description: 'Score 90% on 10 quizzes', unlocked: '1,022', icon: Target, color: 'purple' },
  { id: 4, title: 'Streak keeper', description: 'Maintain a 7-day learning streak', unlocked: '2,480', icon: Flame, color: 'orange' },
  { id: 5, title: 'Elite ranked', description: 'Reach the top 10% on the leaderboard', unlocked: '612', icon: Crown, color: 'yellow' },
  { id: 6, title: 'Perfect score', description: 'Get 100% on any quiz', unlocked: '1,740', icon: CheckCircle, color: 'green' },
  { id: 7, title: 'Prolific creator', description: 'Publish 20 resources', unlocked: '384', icon: FileText, color: 'blue' },
  { id: 8, title: 'Path pioneer', description: 'Publish 5 learning paths', unlocked: '196', icon: MapPin, color: 'purple' },
  { id: 9, title: 'Quality curator', description: '90%+ approval rate', unlocked: '758', icon: ThumbsUp, color: 'green' },
  { id: 10, title: 'Community favourite', description: 'Reach 300+ upvotes', unlocked: '421', icon: Heart, color: 'red' },
];
...
export default function Badges() {
  ...
  const [newBadge, setNewBadge] = useState({ title: '', description: '', unlocked: '0', color: 'blue' });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex gap-2">
          <Button onClick={() => setShowAddModal(true)}><Plus size={18} /> New Badge</Button>
        </div>
      </div>
            ...
            return (
              <Card key={badge.id} className={`hover:shadow-md transition-shadow border-l-4 ${getBorderColor(badge.color)}`}>
                ...
                <Card.Footer className="flex justify-end gap-2">
                  <Button variant="ghost" size="sm" onClick={() => handleEdit(badge.id)}><Edit size={16} /></Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDelete(badge.id)}><Trash2 size={16} /></Button>
                </Card.Footer>
              </Card>
            );
          })
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6 mx-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button variant="primary" className="flex-1" onClick={handleAddBadge}>Add Badge</Button>
              </div>
            </div>
          </div>
        </div>
